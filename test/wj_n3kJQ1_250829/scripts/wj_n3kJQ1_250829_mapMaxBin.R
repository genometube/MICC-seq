...
treatment_pairs <- lapply(treatments[treatments != "n3k_ctrl"], function(treatment) {
    c("n3k_ctrl", treatment)
})
# Create list of scatter plots comparing each treatment to n3k_ctrl
plot_list <- map(treatment_pairs, function(pair) {
    ggplot(wide_data, aes(x = log10(get(pair[1]) + 1), 
                         y = log10(get(pair[2]) + 1))) +
        geom_hex(bins = 30) +
        geom_smooth(method = "lm", se = FALSE, color = "red") +
        labs(x = paste0("Control (", pair[1], ")"), 
             y = paste0("Treatment (", pair[2], ")")) +
        theme_classic() +
        theme(axis.text = element_text(size = 8))
})

# Arrange plots in a grid
panel_plot <- wrap_plots(plot_list, ncol = 3) + 
    plot_annotation(title = "Treatment vs Control Comparisons")
print(panel_plot)
...
head(bedtoolsr::bt.map(a=h3k4me1_bed_Micc1D[,1:3],b=bdg,c=4,o=max))

head(bedtoolsr::bt.map(a=SE[,1:3],b=bdg_files[1],c=4,o=max))
