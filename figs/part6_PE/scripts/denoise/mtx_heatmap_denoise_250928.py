...
def enhance_line_pattern_mrf(image_data, weight=10, max_iter=20):
    """
    Use TV denoising from scikit-image as alternative to MRF
    Parameters:
    - image_data: Input image data (2D array)
    - weight: Denoising weight (higher = smoother)
    - max_iter: Maximum number of iterations
    Returns:
    - Denoised image data
    """
    try:
        print("Using TV denoising as MRF alternative...")
...
def test_denoise_parameters(matrix_subset, output_image=None):
    ...
    try:
        ...
        param_combinations = [
            {'weight': 30, 'max_iter': 10, 'eps': 0.01, 'isotropic': False},
            {'weight': 30, 'max_iter': 20, 'eps': 0.001, 'isotropic': False},
            {'weight': 20, 'max_iter': 30, 'eps': 0.0001, 'isotropic': False},
            {'weight': 20, 'max_iter': 20, 'eps': 0.001, 'isotropic': False}
        ]
        ...
        for i, params in enumerate(param_combinations, 1):
            # Apply denoising
            denoised = denoise_tv_bregman(
                cv2.normalize(matrix_subset, None, 0, 1, cv2.NORM_MINMAX).astype(np.float32),
                weight=params['weight'],
                max_num_iter=params['max_iter'],
                eps=params['eps'],
            ...
            plt.colorbar()
            plt.title(f"Weight: {params['weight']}, Iter: {params['max_iter']}\nEps: {params['eps']}, Iso: {params['isotropic']}")
...
if __name__ == "__main__":
    ...
    output_image = 'matrix_heatmap_14800_15000_denoiseMRF.png'  # 输出图像文件路径
