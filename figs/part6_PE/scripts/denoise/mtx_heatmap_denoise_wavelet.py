...
def enhance_line_pattern_mrf(image_data, weight=0.1, max_iter=None):
    """
    Use wavelet denoising from scikit-image
    Parameters:
    - image_data: Input image data (2D array)
    - weight: Denoising weight (higher = more aggressive denoising)
...
def test_denoise_parameters(matrix_subset, output_image=None):
    ...
    try:
        ...
        param_combinations = [
            {'weight': 0.05, 'wavelet': 'sym8', 'mode': 'soft'},
            {'weight': 0.1, 'wavelet': 'sym8', 'mode': 'soft'},
            {'weight': 0.05, 'wavelet': 'db1', 'mode': 'soft'},
            {'weight': 0.05, 'wavelet': 'coif1', 'mode': 'soft'}
        ]
        ...
        for i, params in enumerate(param_combinations, 1):
            ...
            plt.colorbar()
            plt.title(f"Sigma: {params['weight']}, Wavelet: {params['wavelet']}\nMode: {params['mode']}")
...
if __name__ == "__main__":
    ...
    output_image = '/research/xieyeming1/proj_2025/MICC_paper/genometube/MICC-seq/figs/part6_PE/scripts/denoise/out_png/matrix_heatmap_14800_15000_denoiseWL.png'  # 输出图像文件路径
