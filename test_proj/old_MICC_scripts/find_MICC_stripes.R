...
#USAGE: Rscript find_extrusion.R -i merged_hg38_chr7.hic -f hic -c 7
#USAGE: Rscript find_extrusion.R -i hg38chr7sim.matrix -f matrix -c 7
...
option_list = list(
  make_option(c("-i", "--input"), type="character", default=NULL, 
    ...
    make_option(c("-c", "--chromosome"), type="character", default="1", 
              help="read a chromosome from hic file, attention: some hic files stores chromosome without the \"chr\" character, default is 1",
              metavar="character")
); 
...
##### Read hic file #####
if (args$format=="hic") {
	hic_df <- strawr::straw("KR", args$input, my_chr, my_chr, "BP", 5000)
	} else {
    ...
    for (j in seq(1,length(num))){                        #loop through each x_peak_contact
        ...
        if ((j==length(num)) | j!=1 & (num[j]<(num_bin*0.5) & num[j+1]<(num_bin*0.5))){    
            ...
            x_extr_len <- x_extr_len %>% add_row(chr1=my_chr,
                                                 ...
                                                 end1=format(start_tmp+5000,scientific=F,trim=T),
						 chr2=my_chr,
                                                 start2=format(end_tmp,scientific=F,trim=T), 
                                                 end2=format(end_tmp+5000,scientific=F,trim=T), 
